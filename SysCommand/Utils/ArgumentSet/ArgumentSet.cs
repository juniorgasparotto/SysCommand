...
namespace SysCommand
{
    public class ArgumentSet : IEnumerable<ArgumentMap>
    {
        ...
        public virtual ParseResult Parse(string[] args)
        {
            ...
            var argumentsRaw = ParserUtils.ParseArgumentsRaw(args, null);
            var parseds = ParserUtils.ParseArgumentMapped(argumentsRaw, false, this.argumentsMap);
        ...
        public EvaluateResult Evaluate(ParseResult parseResult)
        {
            ...
            if (commandParse.IsValid)
            {
                ...
                evaluateResult.Result.AddRange(properties.Select(f => new ArgumentResult(f)));
