...
namespace SysCommand.Evaluation
{
    public class DefaultEvaluationStrategy : IEvaluationStrategy
    {
        ...
        public virtual ParseResult Parse(string[] args, IEnumerable<CommandMap> commandsMap, bool enableMultiAction)
        {
            ...
            var argumentsRaw = ParserUtils.ParseArgumentsRaw(args, allMethodsMaps);

            IEnumerable<ArgumentRaw> initialExtraArguments;
            var methodsParsed = ParserUtils.ParseActionMapped(argumentsRaw, enableMultiAction, allMethodsMaps, out initialExtraArguments).ToList();
        ...
        {
            var parseds = ParserUtils.ParseArgumentMapped(argumentsRaw, commandMap.Command.EnablePositionalArgs, commandMap.Properties);
        ...
        {
            var mainMethod = command.GetType().GetMethods().Where(f => f.Name.ToLower() == ParserUtils.MAIN_METHOD_NAME && f.GetParameters().Length == 0).FirstOrDefault();
            if (mainMethod != null)
