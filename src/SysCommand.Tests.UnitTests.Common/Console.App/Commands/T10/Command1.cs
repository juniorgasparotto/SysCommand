...
namespace SysCommand.Tests.UnitTests.Common.Commands.T10
{
    public class Command1 : Command
    {
        ...
        {
            var cur = this.GetAction(T<int, int, int>());
            return GetDebugName(this.GetActionMap(T<int, int, int>()), cur) + " Level" + cur.Level;
        }

        public string Save(int a, int b)
        {
            var cur = this.GetAction(T<int, int>());
            return GetDebugName(this.GetActionMap(T<int, int>()), cur) + " Level" + cur.Level;
        }
